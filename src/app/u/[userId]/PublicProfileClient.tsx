...
import { Container, Typography, Box, Paper, Avatar, Button, Grid, Chip, Tabs, Tab } from "@mui/material";
import ForumCard from "@/components/forum/ForumCard";
import { toggleFollow } from "@/actions/followActions";
import { motion } from "framer-motion";

export default function PublicProfileClient({ profileUser, isFollowing: initialIsFollowing, isSelf }: { profileUser: any, isFollowing: boolean, isSelf: boolean }) {
    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
      <Paper elevation={0} sx={{ p: 4, borderRadius: 4, border: "1px solid #E2E8F0", mb: 4 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
          <Avatar src={profileUser.image || ""} sx={{ width: 120, height: 120, fontSize: "3rem", bgcolor: "primary.main" }}>
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            <Box sx={{ display: 'flex', gap: 3, mb: 2 }}>
              <Typography variant="body1"><strong>{followerCount}</strong> Followers</Typography>
              <Typography variant="body1"><strong>{profileUser._count.following}</strong> Following</Typography>
              <Typography variant="body1"><strong>{profileUser._count.forums}</strong> Posts</Typography>
            </Box>
      ...
      {tabValue === 0 && (
        profileUser.forums.length > 0 ? (
          <Grid container spacing={3}>
            {profileUser.forums.map((forum: any) => (
      ...
      {tabValue === 1 && isSelf && (
        profileUser.bookmarks?.length > 0 ? (
          <Grid container spacing={3}>
            {profileUser.bookmarks.map((forum: any) => (
              <Grid item xs={12} sm={6} md={4} key={forum.id}>
                <ForumCard forum={forum} />
              </Grid>
            ))}
