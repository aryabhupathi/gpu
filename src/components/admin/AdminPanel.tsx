...
import { Box, Typography, Table, TableBody, TableCell, TableHead, TableRow, IconButton, Tooltip, Chip, Avatar, Alert } from "@mui/material";
...
export default function AdminPanel({ users }: { users: any[] }) {
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom>User Moderation</Typography>
          ...
          <TableBody>
            {users.map((user) => (
              <TableRow key={user.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                ...
                <TableCell>
                  <Chip label={user.warnings} color={user.warnings > 2 ? "error" : "default"} size="small" />
                </TableCell>
                <TableCell>
                  {user.banned ? (
