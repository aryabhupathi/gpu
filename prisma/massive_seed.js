...
async function main() {
  ...
  const forumTemplates = [
    {
      title: "How to learn {topic} fast?",
      desc: "I want to master {topic} in the next 3 months. What are the best resources, courses, or projects I should build to get a job using {topic}?",
    },
    {
      title: "Is {topic} still relevant in 2026?",
      desc: "I keep hearing mixed opinions about {topic}. Some say it is dying, others say it is growing. What is the actual industry trend for {topic} right now?",
    },
    {
      title: "My honest review of {topic} after using it for 2 years",
      desc: "I have been using {topic} in production for a while now. Here are my thoughts on what is great and what completely sucks about it.",
    },
    {
      title: "Best practices for scalable {topic} architecture",
      desc: "When building large scale applications, how do you structure your {topic} codebase? I want to avoid massive technical debt.",
    },
    {
      title: "Help! I am stuck with a weird {topic} bug",
      desc: "Has anyone encountered this strange issue with {topic}? It only happens on production and works perfectly on localhost. I am going crazy.",
    },
  ];
  const createdForums = [];
  for (let i = 0; i < 60; i++) {
    ...
    const template =
      forumTemplates[Math.floor(Math.random() * forumTemplates.length)];
    const numTags = Math.floor(Math.random() * 3) + 1;
    const forumTags = [...createdTags]
  ...
  for (const forum of createdForums) {
    ...
    for (let i = 0; i < numComments; i++) {
      ...
      commentDate.setHours(
        commentDate.getHours() + Math.floor(Math.random() * 48),
      );
